...
- Why will it become better than it is now from using ML : персонализированная рекомендательная система будет чаще выдавать пользователю интересующие (или потенциально интересующие) его товары. Увеличивается удовлетворенность пользователей, их retention. Ожидаем, что пользователь вероятнее будет приобретать релевантный товар, больше тратит в сервисе.

  
- What we will consider the success of the iteration from the business perspective:
  1. Значение целевой метрики (см. вопросы выше) после внедрения выше на X% (оценить возможный эффект). Оценку производим на данных в течение месяца после внедрения.
  2. Значение метрики качества ранжирования с использованием новой (или старой вообще не было?) модели > значение метрики с использованием старой (так была или нет?))).

  Сопутствующие вопросы:
    - Выбрать метрику. Видимо, что-то из MAP@k, AP@k, MRR, NDCG. Выбрать метрику и обосновать.
